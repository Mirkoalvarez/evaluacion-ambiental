...
// Si no hay token, ignoramos cualquier usuario guardado (evita loops de sesión inválida)
if (!savedToken) {
    savedUser = null;
...
// Crear usuario desde admin (no altera sesión actual)
...
export const updateUserRemote = createAsyncThunk(
    'auth/updateUser',
    async ({ id, username, email, role, password }, { rejectWithValue }) => {
        try {
...
export const deleteUserRemote = createAsyncThunk(
    'auth/deleteUser',
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(register.rejected, (state, action) => {
                state.status = 'failed';
                state.error = action.payload?.error || 'Error de registro';
            })
            ...
            .addCase(fetchUsers.rejected, (state, action) => {
                ...
                state.usersError = action.payload?.error || 'Error al obtener usuarios';
            })
