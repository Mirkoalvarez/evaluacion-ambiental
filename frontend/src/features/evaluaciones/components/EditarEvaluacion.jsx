...
import { obtenerEvaluacion, patchEvaluacion } from '../slice';
import { updateBarrioNombre, listarBarrios } from '../../barrios/slice';
...
export default function EditarEvaluacion() {
    ...
    useEffect(() => {
        if (actual && String(actual.id) === String(id)) {
            ...
            const f = {}; CAMPOS.forEach(k => { f[k] = actual[k] ?? ''; });
    ...
    const onChange = (e) => {
        ...
        setForm(prev => ({ ...prev, [name]: name === 'r3_3' ? (value === '' ? '' : Number(value)) : value }));
    };
    ...
    const renderNumber = (name, props = {}) => (
        <div className="question-container d-flex flex-column mb-3">
        <label className="form-label mb-1 min-h-label">{LABELS[name]}</label>
        <input className="form-control" type="number" name={name} value={form[name] ?? ''} onChange={onChange} min={props.min ?? 0} step={props.step ?? 1} />
        </div>
    );
    ...
    const onSubmit = async (e) => {
        ...
        try {
            ...
            const cambios = {};
            if (fecha && fecha !== (actual.fecha ?? '')) cambios.fecha = fecha;
    ...
    if (cargando || !actual) return <div className="container"><p>{cargando ? 'Cargando…' : (error || 'Sin datos')}</p></div>;
    ...
    return (
        ...
        <div className="card mb-3">
            ...
            <div className="row g-2 align-items-end">
                ...
                <input className="form-control" value={barrioNombre} onChange={(e) => setBarrioNombre(e.target.value)} placeholder="Nombre del barrio" />
                <small className="text-muted">Cambia el nombre del barrio aquí. No se toca el barrio_id de la evaluación.</small>
                </div>
                <div className="col-md-6">
        ...
        <form className="row g-3" onSubmit={onSubmit}>
            <div className="col-12 mt-4">
                <div className="section-energia">
                    ...
                    {['e1_1','e1_2','e1_3'].map(n => (<div className="col-md-4" key={n}>{renderQuestion(n, SINO)}</div>))}
                    {['e1_4','e1_5'].map(n => (<div className="col-md-4" key={n}>{renderQuestion(n, NPT)}</div>))}
                    {['e1_6','e1_7','e1_8','e1_9'].map(n => (<div className="col-md-4" key={n}>{renderQuestion(n, SINO)}</div>))}
                    </div>
                </div>
            </div>
            ...
            <div className="col-12 mt-4">
                <div className="section-agua">
                    ...
                    <div className="row g-3">
                    {['a2_1','a2_2'].map(n => (<div className="col-md-4" key={n}>{renderQuestion(n, SINO)}</div>))}
                    {['a2_3','a2_4'].map(n => (<div className="col-md-4" key={n}>{renderQuestion(n, n === 'a2_4' ? NPT_NA : NPT)}</div>))}
                    {['a2_5','a2_6','a2_7','a2_8'].map(n => (<div className="col-md-4" key={n}>{renderQuestion(n, ['a2_5','a2_6'].includes(n) ? SINO_NA : SINO)}</div>))}
                    </div>
                </div>
            </div>
            ...
            <div className="col-12 mt-4">
                <div className="section-residuos">
                    ...
                    <div className="row g-3">
                    {['r3_1','r3_2'].map(n => (<div className="col-md-4" key={n}>{renderQuestion(n, SINO)}</div>))}
                    <div className="col-md-4">{renderNumber('r3_3', { min: 0, step: 1 })}</div>
                    {['r3_4','r3_5','r3_6'].map(n => (<div className="col-md-4" key={n}>{renderQuestion(n, SINO)}</div>))}
                    </div>
                </div>
            </div>
            ...
            <div className="col-12 mt-4">
                <div className="section-ev">
                    ...
                    {['ev4_2','ev4_3','ev4_4','ev4_5','ev4_6','ev4_7','ev4_8'].map(n => (<div className="col-md-4" key={n}>{renderQuestion(n, SINO)}</div>))}
                    </div>
                </div>
            </div>
            ...
            <div className="col-12 mt-4">
                <div className="section-gestion">
                    ...
                    <div className="row g-3">
                    {['g5_1','g5_2','g5_3','g5_4','g5_5','g5_6'].map(n => (<div className="col-md-4" key={n}>{renderQuestion(n, SINO)}</div>))}
                    </div>
                </div>
            </div>
